#!/bin/bash
# Generate API documentation and build HTML docs for ReactorModels

set -euo pipefail

DOCS_DIR="docs"
API_DOCS_DIR="${DOCS_DIR}/api"
SRC_DIR="src/reactormodels"

echo "Cleaning previous API docs in ${API_DOCS_DIR}..."
rm -fv "${API_DOCS_DIR}"/reactormodels.*.rst "${API_DOCS_DIR}"/modules.rst || true
