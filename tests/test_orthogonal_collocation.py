...
=======
# ---------------------------------------------------------------------------
# Additional coverage: construction edge cases, domain-length scaling, and
# branches (r=0 special case, public radial_operator()) not exercised above.
# All existing tests use the default domain_length=1.0 and add_inlet=True,
# so anything specific to *those* defaults being varied lives here.
# ---------------------------------------------------------------------------


def test_domain_length_must_be_positive():
    """_build() raises for domain_length <= 0 -- untested branch."""
    with pytest.raises(ValueError):
        reactormodels.numerics.OrthogonalCollocation(
            domain_length=0.0, n_interior_points=4
        )

    with pytest.raises(ValueError):
        reactormodels.numerics.OrthogonalCollocation(
            domain_length=-1.0, n_interior_points=4
        )


def test_add_inlet_prepends_zero_node():
    """With add_inlet=True, x=0 should be the first node and prepended
    relative to add_inlet=False -- nothing above compares the two."""
    with_inlet = reactormodels.numerics.OrthogonalCollocation(
        n_interior_points=4, add_inlet=True
    )
    without_inlet = reactormodels.numerics.OrthogonalCollocation(
        n_interior_points=4, add_inlet=False
    )

    assert with_inlet.nodes[0] == pytest.approx(0.0, abs=1e-14)
    assert without_inlet.nodes[0] > 0.0
    assert len(with_inlet.nodes) == len(without_inlet.nodes) + 1


def test_domain_length_scales_derivative_matrices():
    """Differentiation matrices are built on [0,1] then rescaled -- should
    scale as 1/L and 1/L^2. Nothing above uses domain_length != 1.0."""
    col_unit = reactormodels.numerics.OrthogonalCollocation(
        domain_length=1.0, n_interior_points=4
    )
    col_scaled = reactormodels.numerics.OrthogonalCollocation(
        domain_length=4.0, n_interior_points=4
    )

    np.testing.assert_allclose(
        col_scaled.first_derivative, col_unit.first_derivative / 4.0, atol=1e-10
    )
    np.testing.assert_allclose(
        col_scaled.second_derivative, col_unit.second_derivative / 16.0, atol=1e-10
    )


def test_integrate_scales_with_domain_length():
    """Same quadrature checks as test_integrate_constant/linear, but at
    L != 1, so a bug in the domain_length rescaling of weights (self.weights
    * self.domain_length in _build) can't hide behind L=1."""
    L = 3.0
    oc = reactormodels.numerics.OrthogonalCollocation(
        domain_length=L, n_interior_points=5
    )
    ones = np.ones(len(oc.nodes))
    assert oc.integrate(ones) == pytest.approx(L, rel=1e-8)
    assert oc.integrate(oc.nodes) == pytest.approx(L**2 / 2, rel=1e-6)


def test_second_derivative_matrix_equals_first_derivative_squared():
    """For a single element, applying the first-derivative matrix twice
    should equal the second-derivative matrix directly (B = A @ A in the
    underlying Lagrange construction). Only holds for a single element --
    multi-element matrices have junction-averaging that breaks this."""
    oc = reactormodels.numerics.OrthogonalCollocation(
        domain_length=2.0, n_interior_points=5, n_elements=1
    )
    composed = oc.first_derivative @ oc.first_derivative
    np.testing.assert_allclose(composed, oc.second_derivative, atol=1e-8)


def test_radial_operator_matches_known_function():
    """test_radial_operator_constant only checks the Laplacian of a
    constant (=0), which doesn't exercise the (2/r)*df/dr term at all. For
    f(r) = r^2, the spherical Laplacian d^2f/dr^2 + (2/r)df/dr = 6
    everywhere, including at r=0 (the removable-singularity '3*f' branch)."""
    oc = reactormodels.numerics.OrthogonalCollocation(n_interior_points=5, n_elements=1)
    f = oc.nodes**2

    lap = oc.evaluate_radial_operator(f)
    np.testing.assert_allclose(lap, 6.0, atol=1e-8)

    # node=int branch, including node=0 (the r=0 special case)
    for i in range(len(oc.nodes)):
        val = oc.evaluate_radial_operator(f, node=i)
        assert val == pytest.approx(6.0, abs=1e-8)


def test_radial_operator_public_method_matches_stored_matrix():
    """radial_operator() re-derives the matrix already stored as
    self.radial_operator_matrix in __init__ -- nothing checks they agree."""
    oc = reactormodels.numerics.OrthogonalCollocation(n_interior_points=4, n_elements=1)
    np.testing.assert_allclose(
        oc.radial_operator(), oc.radial_operator_matrix, atol=1e-12
    )
>>>>>>> 59474be (add tests to increase coverage)
